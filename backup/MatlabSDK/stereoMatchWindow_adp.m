function [D1,D2] = stereoMatchWindow_adp(I1, I2, ws, max_d,thresh,lambda_C,lambda_AD)
%Image allignment
[~,cols,~] = size(I1);
[rows,cols2,~] = size(I2);
if cols > cols2
...
%     match_cost_bit_bin = zeros(rows + 2*max_window_radius,cols - d + 2*max_window_radius);
    ...
    match_cost_bit_sq = match_cost_bit_AD.^2;
    intgerated_match_cost_sq = zeros(size(match_cost_bit_AD,1)+1,size(match_cost_bit_AD,2)+1);
...
windowed_match_cost_AD = zeros(size(match_cost_bit_AD,1),size(match_cost_bit_AD,2),length(ws));
windowed_match_cost_sq = zeros(size(match_cost_bit_AD,1),size(match_cost_bit_AD,2),length(ws));
windowed_match_cost_var = zeros(size(match_cost_bit_AD,1),size(match_cost_bit_AD,2),length(ws));
windowed_match_cost = zeros(size(match_cost_bit_AD,1),size(match_cost_bit_AD,2),length(ws));
for r = 1:length(ws)
    ...
    windowed_match_cost_AD(:,:,r) =  windowed_match_cost_AD(:,:,r)/(ws(r)^2);
    windowed_match_cost_var(:,:,r) = windowed_match_cost_sq(:,:,r)-windowed_match_cost_AD(:,:,r).^2;
end
...
%    ra2 = max_window_radius+1:size(windowed_match_cost_C,2)-max_window_radius;
    windowed_match_cost(ws(r)+1:end-ws(r),ws(r)+1:end-ws(r),r) = windowed_match_cost_AD(ws(r)+1:end-ws(r),ws(r)+1:end-ws(r),r) +1.5*windowed_match_cost_var(ws(r)+1:end-ws(r),ws(r)+1:end-ws(r),r) +7/(ws(r)-2);
    ra1 = max_window_radius+1:size(match_cost_bit_AD,1)-max_window_radius;
    ra2 = max_window_radius+1:size(match_cost_bit_AD,2)-max_window_radius;
    match_cost = min(windowed_match_cost(ra1,ra2,:),[],3) ;
