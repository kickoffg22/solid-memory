function [D1,D2] = stereoMatchWindowCensus_adp(I1, I2, max_window_radius,min_window_radius, max_d,thresh,lambda_C,lambda_AD)
%Image allignment
[~,cols,~] = size(I1);
[rows,cols2,~] = size(I2);
if cols > cols2
...
%     windowed_match_cost_AD = intgerated_match_cost_AD(2*max_window_radius+2:end,2*max_window_radius+2:end)...
%         - intgerated_match_cost_AD(1:end-(2*max_window_radius+1),2*max_window_radius+2:end)...
%         - intgerated_match_cost_AD(2*max_window_radius+2:end,1:end-(2*max_window_radius+1))...
%         + intgerated_match_cost_AD(1:end-(2*max_window_radius+1),1:end-(2*max_window_radius+1));
...
%     match_cost = match_cost(max_window_radius+1:end-max_window_radius,max_window_radius+1:end-max_window_radius);
   ...
   ra2 = max_window_radius+1:size(windowed_match_cost_C,2)-max_window_radius;
    match_cost = windowed_match_cost_C(ra1,ra2,1) .* F(:,range1) +windowed_match_cost_C(ra1,ra2,1).*(~F(:,range1));
    C1c = C1min(:,range1);
    C2c = C2min(:,range2);
    D1c = D1(:,range1);
    D2c = D2(:,range2);
