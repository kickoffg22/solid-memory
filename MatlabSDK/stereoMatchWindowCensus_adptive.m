function [D1,D2] = stereoMatchWindowCensus_adptive(I1, I2, window_radius, max_d)
%Image allignment
max_window = 8;
min_window = 2;
[~,cols,~] = size(I1);
...
%Census Transform
% IC1 = Adp_CT(I1,'window_long',17,'window_short',3);%Census tranformation
% IC2 = Adp_CT(I2,'window_long',17,'window_short',3);
...
D1 = zeros(rows,cols);
D2 = zeros(rows,cols);
total_windowed_match_cost = Inf(rows,cols,2);
for d = 1:max_d
    ...
    %Average AD color matching cost    
...
%     windowed_match_cost_C = windowed_match_cost_C(min(windowed_match_cost_C(:)));
    total_windowed_match_cost(:,range1,i) = (2-exp(-2*windowed_match_cost_C/max(windowed_match_cost_C(:)))-exp(-2*windowed_match_cost_AD/max(windowed_match_cost_AD(:))))/(2*window_radius+1)^2;
    ...
    match_cost = min(total_windowed_match_cost(:,range1,:),[],3);
