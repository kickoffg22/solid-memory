...
function C = CCT(I,varargin)
fprintf('Performing Color Invariant Census Transform');
p = inputParser;
...
if channels == 3
        ...
        E1 = 0.299*(0.06*R+0.63*G+0.27*B);
        E2 = 0.587*(0.3*R+0.04*G-0.35*B);
        E3 = 0.114*(0.34*R-0.6*G+0.17*B);
        I_GCM = zeros(size(I));
        I_GCM(:,:,1) = E1;
        I_GCM(:,:,2) = E2;
        I_GCM(:,:,3) = E3;
...
power = window_length*window_width;%Element number in the window, also used for bitset
shifteded_I = zeros(size(I_GCM));
i_max = (window_length-1)/2;
j_max = (window_width-1)/2;
I_mean = zeros(size(I_GCM,1),size(I_GCM,2));
for i = -i_max:i_max
    for j = -j_max:j_max
        shifteded_I(:,:,:) = 0;
        if i <= 0 && j <= 0 %comparing to upper left
            shifteded_I(1:end+i,1:end+j,:) = I_GCM(1-i:end,1-j:end,:);
        elseif i <= 0 && j >= 0%comparing to upper right
            shifteded_I(1:end+i,1+j:end,:) = I_GCM(1-i:end,1:end-j,:);
        elseif i >= 0 && j <= 0%comparing to bottom left
            shifteded_I(1+i:end,1:end+j,:) = I_GCM(1:end-i,1-j:end,:);
        elseif i >= 0 && j >= 0%comparing to bottom right
            shifteded_I(1+i:end,1+j:end,:) = I_GCM(1:end-i,1:end-j,:);
        end
        I_mean = I_mean + sqrt(sum((shifteded_I - I_GCM).^2,3));        
    end
end
I_mean = I_mean/power;
...
for i = -i_max:i_max
    for j = -j_max:j_max
        ...
        I_dist = sqrt(sum((I_GCM - shifteded_I).^2,3));
        C(I_mean <I_dist) = bitset(C(I_mean < I_dist),power);
        power = power - 1;
    end
end
end